...
const BottomSheetComponent = /*#__PURE__*/(0, _react.forwardRef)(function BottomSheet(props, ref) {
  ...
  const getEvaluatedPosition = (0, _reactNativeReanimated.useWorkletCallback)(function getEvaluatedPosition(source) {
     ...
     */
    const resolveSnapPointByIndex = targetIndex => {
      'worklet';

      const isValidIndex = targetIndex >= 0 && targetIndex < snapPoints.length;
      return isValidIndex ? snapPoints[targetIndex] : undefined;
    };
    const resolvedCurrentIndex = resolveSnapPointByIndex(currentIndex);
    if (resolvedCurrentIndex !== undefined) {
      return resolvedCurrentIndex;
    }

    /**
     * if current animation position matches the closed position, keep it closed.
     */
    const isCurrentlyClosed = Math.abs(animatedPosition.value - animatedClosedPosition.value) <= 0.5;
    if (isCurrentlyClosed) {
      return animatedClosedPosition.value;
    }

    /**
     * fallback to the provided index if it is valid.
     */
    const resolvedProvidedIndex = resolveSnapPointByIndex(_providedIndex);
    if (resolvedProvidedIndex !== undefined) {
      return resolvedProvidedIndex;
    }

    /**
     * fallback to the last snap point or current animated position to avoid undefined values.
     */
  ...
  const evaluatePosition = (0, _reactNativeReanimated.useWorkletCallback)(function evaluatePosition(source, animationConfigs) {
     ...
     */
    if (animatedAnimationState.value !== _constants.ANIMATION_STATE.RUNNING && animatedCurrentIndex.value === -1 && animatedNextPositionIndex.value === -1) {
      /**
