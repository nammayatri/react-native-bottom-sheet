...
const useAnimatedSnapPoints = (snapPoints, containerHeight, contentHeight, handleHeight, footerHeight, enableDynamicSizing, maxDynamicContentSize) => {
  ...
  const normalizedSnapPoints = (0, _reactNativeReanimated.useDerivedValue)(() => {
    ...
    const cappedContentHeight = Math.min(contentHeight.value + handleHeight.value + footerHeight.value, maxDynamicContentSize !== undefined ? maxDynamicContentSize : containerHeight.value);
    let dynamicSnapPoint = containerHeight.value - cappedContentHeight;
    if (dynamicSnapPoint === _constants.INITIAL_SNAP_POINT) {
